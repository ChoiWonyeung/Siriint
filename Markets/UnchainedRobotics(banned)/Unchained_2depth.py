...
def getPost(ls_source) -> list:
    ...
    for post in ls_source:
        ...
        print(dic_unchained)
        counter = counter + 1
    pickle_save('pickle/dic_amazon_2depth.pkl', dic_unchained)
    pd.DataFrame(dic_unchained).to_csv("data/amazon.csv",encoding='utf-8-sig')
    json_save("data/amazon.json", dic_unchained)
