    ...
    BWR_json =\
            {
                ...
                'collections': [String],  # 상품이 소속된 콜렉션 목록 # unique # ls_len[,10]
                'catalogs':  # 상품 카탈로그 목록
                    [
                        {
                            'title': {'{lang}': String or None},  # 카탈로그 제목 # {lang}len[0, 100]
                            'description': {'{lang}': String or None},  # 카탈로그 설명 # {lang}len[0,10000]
                            'image': String or None  # 카탈로그 이미지
                        }
                    ],
                'thumbnail': String or None,  # 상품 썸네일
                'name': {'{lang}': String or None},  # 상품의 이름 # 필수
                'summary': {'{lang}': String or None},  # 상품의 요약 설명
                'description': {'{lang}': String or None},  # 상품에 대한 설명 # {lang}len[0, 50000]
                'notices':       # 상품의 알림성 컨텐츠
                    [
                        {
                            'slug': String,  # 알림성 컨텐츠 슬러그 # 한국의 상품 고시 정보는 "legal-KR_"로 시작하는 슬러그로 예약 사용
                            'title': {'{lang}': String or None},  # 알림성 컨텐츠 제목 # {lang}len[0, 100]
                            'items':  # 알림성 컨텐츠 세부 목록
                                [
                                    {
                                        'title': {'{lang}': String or None},  # 세부 목록의 제목 # {lang}len[0, 100]
                                        'description': {'{lang}': String or None}  # 세부 컨텐츠의 설명 # {lang}len[0, 10000]
                                    }
                                ]
                        }
                    ],
                ...
                'taxCategories': [String],  # 상품이 소속된 세금 카테고리 목록 # unique # len[,10]
                'shipping':  # 상품 배송 설정 # 필수 # string or 'tangible'
                    {
                        'methods': [String],  # 상품이 지원하는 배송 방식 목록
                        'calculation': String or 'bundled' or 'separated' or None  # 상품의 계산 방식 # 필수
                    },
                'options':  # 상품의 옵션 len[,5]
                    [
                        {
                            'name': {'{lang}': String or None},  # 옵션 이름 # {lang}len[0,50]
                            'priority': Number,  # 옵션의 정렬 순서 # Integer_len[0,4]
                            'variations':  # 옵션의 선택지 목록 # len[,120]
                                [
                                    {
                                        'value': {'{lang}': String or None},  # 옵션의 선택지 값 # {lang]len[0,150]
                    ...
                    [
                        {
                            ...
                            'name': {'{lang}': String or None},  # 추가 구매 옵션의 이름 # {lang}len[0,100]
                            'items':  # 추가 구매 옵션의 품목
                                [
                                    {
